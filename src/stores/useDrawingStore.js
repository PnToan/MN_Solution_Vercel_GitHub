...
const store = createSimpleStore({
  ...
  startPanelEdit(panelId = null, shapeTool = null) {
    ...
    const context = buildPanelEditContext(panel, state.panelEdit?.context?.faceSide || null)
  ...
  //=================
  setPanelEditFaceSide(faceSide) {
    if (!state.panelEdit?.active || !state.panelEdit?.selectedPanelId) return null

    const panel = state.panels.find((item) => item.id === state.panelEdit.selectedPanelId) || null
    const context = buildPanelEditContext(panel, faceSide)

    if (!context) return state.panelEdit.context
