...
//=================
function drawCircleLocal(ctx, viewport, center, radius, options = {}) {
  if (!center || !Number.isFinite(Number(radius)) || Number(radius) <= 0) return

  const screenCenter = localToScreen(viewport, center.x, center.y)
  const screenRadiusPoint = localToScreen(viewport, center.x + Number(radius), center.y)
  const screenRadius = Math.abs(screenRadiusPoint.x - screenCenter.x)

  ctx.save()
  ctx.strokeStyle = options.stroke || '#111111'
  ctx.fillStyle = options.fill || 'rgba(0, 0, 0, 0.02)'
  ctx.lineWidth = options.lineWidth || 1.5
  ctx.beginPath()
  ctx.arc(screenCenter.x, screenCenter.y, screenRadius, 0, Math.PI * 2)
  if (options.fill !== null) ctx.fill()
  ctx.stroke()
  ctx.restore()
} // End drawCircleLocal

//=================
function drawPanelEditSavedCircles(ctx, viewport, panel, panelRect, currentView) {
  ...
  Object.values(data).forEach((faceData) => {
    ...
    const circles = Array.isArray(faceData.circles) ? faceData.circles : []

    circles.forEach((circle) => {
      const center = getPanelEditLocalPoint(panel, circle.center, faceData.faceKey, panelRect, currentView)

      drawCircleLocal(ctx, viewport, center, Number(circle.radius || 0), {
        stroke: '#111111',
