...
function drawPanelEditCanvas(editContext = null, width = null, height = null) {
  ...
  const textColor = getCssVariable('--mn-text-main', '#111111')
  const gridColor = getCssVariable('--mn-border-main', '#d7dbe0')
  const margin = 44
  ...
  const offsetX = (canvasWidth - faceWidth) / 2
  const offsetY = (canvasHeight - faceHeight) / 2
  const bottomLeft = getPanelEditPoint(context, offsetX, offsetY, scale, 0, 0)
  ...
  targetContext.fillRect(offsetX, offsetY, faceWidth, faceHeight)
  targetContext.restore()
  targetContext.strokeStyle = borderColor
  targetContext.lineWidth = 2
  targetContext.strokeRect(offsetX, offsetY, faceWidth, faceHeight)
  targetContext.strokeStyle = gridColor
  targetContext.lineWidth = 1
  targetContext.setLineDash([4, 4])
  targetContext.beginPath()
  targetContext.moveTo(bottomLeft.x, bottomLeft.y)
  targetContext.lineTo(offsetX + faceWidth, bottomLeft.y)
  targetContext.moveTo(bottomLeft.x, bottomLeft.y)
  targetContext.lineTo(bottomLeft.x, offsetY)
  ...
  targetContext.fillText('0,0', bottomLeft.x + 6, bottomLeft.y - 6)
  targetContext.textAlign = 'center'
  targetContext.textBaseline = 'top'
  targetContext.fillText(`${Math.round(context.width)} x ${Math.round(context.height)} mm`, offsetX + faceWidth / 2, offsetY + faceHeight + 10)
